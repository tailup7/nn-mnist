...
$\mathbf{b}_1 \in \mathbb{R}^{n_1}$ はバイアスベクトルである。 同様に、 $\mathrm{W}_2$は $n_2 \times n_1$行列、 $\mathbf{b}_2 \in \mathbb{R}^{n_2}$ (中間層2のノード数を $n_2$とし、今回は $n_2=64$である)、 $\mathrm{W}_3$は $n \times n_2$行列、 $\mathbf{b}_3 \in \mathbb{R}^{n}$ (出力層のノード数を $n$とし、今回は 0 ~ 9 の数字に分類するので $n=10$)
...
また、 $\mathrm{softmax}$ 関数は、出力値を確率に変換するための正規化関数であり、ベクトル $\mathbf{y} = (y_1, y_2, ... , y_n)$ の各成分に対して以下の式で定義される。
...
$$
L = - \sum_{j=1}^n t_j \log \hat{y}_j
$$

ここで、 $t_j$ は正解ラベルであり、 $t_j (j = 1,2, ..., n)$ のいずれか1つだけが1で、それ以外が0であるようなベクトル(ワンホットベクトル)である。また、 $\hat{y}_j$ はクラス $j$ に対する予測確率を表す。
